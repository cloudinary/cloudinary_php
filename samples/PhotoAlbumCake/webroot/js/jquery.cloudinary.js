...
(function( $ ) {
  ...
  function getInitialValue(el) {
    var $el = $(el);
    if (!$el.data('cloudinaryField')) { return; }
    var widget_parts = $el.prevUntil("[for=" + $el.attr('id') + "]");
    return widget_parts.filter('.initial-value');
  }
  function handleDone(e, data) {
    ...
    if (data.cloudinaryField && data.form.length > 0) {
      ...
      var initial_value = getInitialValue(this);
      if (initial_value.length == 1) {
        initial_value.find('a').attr('disabled', false).attr('href', data.result.url).text(data.result.public_id);
      }
    }
  ...
  function handleStart(e, data) {
    ...
    var initial_value = getInitialValue(this);
    if (initial_value.length != 1) { return; }
    initial_value.find('a').attr('disabled', true).text('Uploading...');
  }
  $.fn.cloudinary_fileupload = function(options) {
    ...
    if (initializing) {
      this.bind("fileuploaddone", handleDone);
      this.bind("fileuploadstart", handleStart);
