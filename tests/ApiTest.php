...
namespace Cloudinary {
    ...
    class ApiTest extends TestCase
    {
         ...
         */
        protected static function delete_resources(\Cloudinary\Api $api)
        {
            try {
         ...
         * @param \Cloudinary\Api $api an initialized api object
         */
        protected static function delete_transformations(\Cloudinary\Api $api)
        {
        ...
        public function test32_folder_listing()
        {
            $this->markTestSkipped(
                ...
                " and the account should be empty of folders. Comment out this line if you really want to test it."
            );
        ...
        public function test_update_delete_streaming_profile()
        {
            ...
            try {
                $this->api->create_streaming_profile($name, array(
                    "representations" => array(
                        array(
                            "transformation" => array(
                                "bit_rate" => "5m",
        ...
        /**
         * @dataProvider updateParametersProvider
         *
         ...
         */
        public function test_update_parameters(array $expected, array $params)
        {
            Curl::mockApi($this);

            $this->api->update(self::$api_test, $params);
            assertUrl($this, '/resources/image/upload/' . self::$api_test);
            $fields = Curl::$instance->fields();
            foreach ($expected as $key => $expectation) {
                if ($expectation) {
                    $this->assertArrayHasKey($key, $fields, "update() should not send empty parameters");
                } else {
                    $this->assertArrayNotHasKey($key, $fields, "update() should not send empty parameters");
                }
            }
        }
         ...
         * @return array
         */
        public function updateParametersProvider()
        {
            return array(
                'empty' => array(
                    'expected' => array(
                        'face_coordinates' => false,
                        'tags' => false,
                        'context' => false,
                        'custom_coordinates' => false,
                        'notification_url' => false,
                    ),
                    'params' => array("auto_tagging" => 0.5),
                ),
                'face_coordinates' => array(
                    ...
                    'params' => array('face_coordinates' => true),
                ),
                'tags' => array(
                    'expected' => array(
                        'face_coordinates' => false,
                ...
                'custom_coordinates' => array(
                    ...
                    'params' => array('custom_coordinates' => 'custom_coordinates'),
                ),
                'notification_url' => array(
                    ...
                    'params' => array('notification_url' => 'notification_url'),
                ),
            );
        }
    }
