 ...
 */
class FileSystemKeyValueStorageTest extends UnitTestCase
{
    /**
    ...
    public function setUp()
    {
        ...
        $this->rootPath = sys_get_temp_dir() . DIRECTORY_SEPARATOR . uniqid(self::$UNIQUE_TEST_ID . '_', true);
    ...
    {
        try {
            $getKeyFullPath = new ReflectionMethod(
                FileSystemKeyValueStorage::class,
                'getKeyFullPath'
            );
        } catch (Exception $e) {
            self::fail("Failed getting key full path: $e");

            return false; // actually we never get here
        }
    ...
    {
        $nonExistingRootPath = sys_get_temp_dir() . DIRECTORY_SEPARATOR . uniqid(self::$UNIQUE_TEST_ID . '_', true);
